...
function Hints(props) {
    ...
    return (
        ...
        <div className="">
        <button className="button is-small hint-btn m-1" onClick={handleFifty} disabled={fFState ? true : false}>50/50</button>
            ...
            <button className="button is-small hint-btn m-1" onClick={handleSize} disabled={sizeState ? true : false}>Dog Size</button>
            <button className="button is-small hint-btn m-1" onClick={selectLetters} disabled={letterClue.used ? true : false}>2 Letter</button>
        </div>
        </div>
    )
}
